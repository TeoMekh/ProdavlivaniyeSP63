    ...
    <div className="workspace">
      ...
      <section className="results">
        ...
        <div className="check card"><div className="cardTitle"><div><b>Проверка прочности</b><small>{outerGoverns?"Определяет внешний контур без поперечной арматуры":"Определяет основной расчётный контур"}</small></div><span className={governingEta<=1?"pill green":"pill red"}>{governingEta<=1?"выполнено":"не выполнено"}</span></div>
          ...
          {fswAccepted>0&&<div className="normLimit"><b>Состав несущей способности по п. 8.1.50</b><span>Fult = {fb.toFixed(1)} + {fswAccepted.toFixed(1)} = {fult.toFixed(1)} кН; {!wall&&<>Mₓ,ult = {mbx.toFixed(1)} + {mswx.toFixed(1)} = {mxUlt.toFixed(1)} кН·м; </>}Mᵧ,ult = {mby.toFixed(1)} + {mswy.toFixed(1)} = {myUlt.toFixed(1)} кН·м.</span></div>}
          ...
          {momentLimited&&<div className="normLimit"><b>Сработало ограничение СП 63</b><span>Полный относительный вклад моментов {rawMomentRatio.toFixed(3)} принят равным пределу {acceptedMomentRatio.toFixed(3)} = 0,5·F/Fult.</span></div>}
