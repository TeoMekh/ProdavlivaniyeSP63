...
export async function buildPunchingReport(d: ReportData) {
  ...
  const children: Array<Paragraph | Table> = [
    ...
    explanation("Расчётный контур располагают на расстоянии h₀/2 от границы площадки передачи нагрузки. Для опоры у края или угла плиты контур ограничивается свободными краями. При наличии отверстия в зоне до 6h₀ участок контура между касательными к отверстию исключается."),
    table([["Характеристика", "Значение"], ["Периметр u", `${num(d.u / 1000)} м`], ["Моменты сопротивления Wₓ / Wᵧ", `${num(d.wx / 1e6, 2)} / ${num(d.wy / 1e6, 2)} ·10⁶ мм²`], ["Центр тяжести контура", `x = ${num(d.xc, 0)} мм; y = ${num(d.yc, 0)} мм`], ["Влияние отверстия", d.hole ? d.holeStatus : "Отверстие отсутствует"]]),
    ...
    formula("Aᵦ = u · h₀    (формула 8.89)", `${num(d.u, 0)} · ${num(d.h0, 0)} = ${num(d.u * d.h0, 0)} мм²`),
    explanation("Предельное усилие, воспринимаемое бетоном, определяем по расчётному сопротивлению бетона осевому растяжению."),
    formula("Fᵦ,ult = Rbt · Aᵦ    (формула 8.88)", `${num(d.rbt, 2)} · ${num(d.u * d.h0, 0)} / 1000 = ${num(d.fb, 1)} кН`),
  ];
  ...
  if (d.reinforced) {
    children.push(
      ...
      formula("qsw = Rsw · Asw / sw    (формула 8.92)", `${num(d.rsw, 0)} · ${num(d.asw, 1)} / ${num(d.swStep, 0)} = ${num(d.qsw, 1)} Н/мм`),
      formula("Fsw,ult = 0,8 · qsw · u    (формула 8.91)", `0,8 · ${num(d.qsw, 1)} · ${num(d.u, 0)} / 1000 = ${num(d.fswRaw, 1)} кН`),
      ...
      table([["Проверка пункта 8.1.48", "Результат"], ["Минимальное учитываемое усилие 0,25Fb", `${num(d.fswRaw, 1)} ≥ ${num(d.fswThreshold, 1)} кН`], ["Ограничение Fsw ≤ Fb", `${num(d.fswAccepted, 1)} ≤ ${num(d.fb, 1)} кН`], ["Принято в расчёте", `Fsw = ${num(d.fswAccepted, 1)} кН`]]),
    );
  }
  ...
  children.push(
    ...
    formula("Fult = Fb,ult + Fsw,ult", `${num(d.fb, 1)} + ${num(d.fswAccepted, 1)} = ${num(d.fb + d.fswAccepted, 1)} кН`),
    formula("F / Fult", `${num(d.force, 1)} / ${num(d.fb + d.fswAccepted, 1)} = ${num(d.forceRatio)}`),
    formula("Mᵦ,ult = Rbt · Wᵦ · h₀    (формула 8.94)", `определение предельных моментов по Wₓ и Wᵧ`),
    formula("ΣM / Mult", num(d.rawMomentRatio)),
    formula("0,5 · F / Fult", `0,5 · ${num(d.forceRatio)} = ${num(d.momentLimit)}`),
    formula("η = F/Fult + min(ΣM/Mult; 0,5·F/Fult)", `${num(d.forceRatio)} + ${num(d.acceptedMomentRatio)} = ${num(d.eta)}`),
  ...
  if (d.reinforced) children.push(
    ...
    formula("ηouter = F/Fb,ult + min(ΣM/Mb,ult; 0,5·F/Fb,ult)", `${num(d.outerForceRatio)} + ${num(d.outerMomentRatio)} = ${num(d.outerEta)}`),
  );
  children.push(
    ...
    new Paragraph({ spacing: { after: 80 }, children: [new TextRun({ text: "Расчёт выполнен по СП 63.13330.2018, пункты 8.1.46–8.1.52 и 10.3.17. Учтены ограничение вклада изгибающих моментов, требования к поперечной арматуре и проверка внешнего контура без её учёта.", font: "Arial", size: 20, color: ink })] }),
